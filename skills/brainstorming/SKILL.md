...
- **"能不能" ≠ 执行许可** — 用户问可行性时只回答可行性+方案，等明确指令如"去做"/"开干"才动手。禁止在"能不能"阶段直接修改代码。
- **先搜资源再提案** — 在提出任何实现方案之前，先搜索外部资源(文档、GitHub、API)验证可行性。用户会测试"有没有搜到"。
- **禁止拿半成品当全绿** — 完成标准: 代码写完 + 全链路跑通 + 原始输出作为证据。未验证的代码=未完成。
